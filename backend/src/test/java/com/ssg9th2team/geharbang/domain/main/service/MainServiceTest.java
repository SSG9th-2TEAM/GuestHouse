...
public class MainServiceTest {
    ...
    void setUp() {
        ...
        acc1 = Accommodation.builder().accommodationsId(1L).accommodationsName("오션뷰 파티하우스").rating(4.8).reviewCount(150).build();
        acc2 = Accommodation.builder().accommodationsId(2L).accommodationsName("조용한 오션뷰 숙소").rating(4.5).reviewCount(120).build();
        acc3 = Accommodation.builder().accommodationsId(3L).accommodationsName("숲속의 조용한 집").rating(4.9).reviewCount(200).build();
    ...
    void testRecommendationForUserWithThemes() {
        MainAccommodationListResponse response = mainService.getMainAccommodationList(1L, null);
    ...
    void testRecommendationForUserWithoutThemes() {
        MainAccommodationListResponse response = mainService.getMainAccommodationList(2L, null);
    ...
    void testRecommendationForUnauthenticatedUser() {
        MainAccommodationListResponse response = mainService.getMainAccommodationList(null, null);
        assertThat(response.getRecommendedAccommodations()).isEmpty();
        assertThat(response.getGeneralAccommodations()).hasSize(3);
    }
    ...
    void testFilteringWithThemeIds() {
        Set<Long> filteredIds = Set.of(acc2.getAccommodationsId(), acc3.getAccommodationsId());
        when(accommodationThemeRepository.findAccommodationIdsByThemeIds(any(Set.class))).thenReturn(filteredIds);

        MainAccommodationListResponse response = mainService.getMainAccommodationList(1L, List.of(3L));
