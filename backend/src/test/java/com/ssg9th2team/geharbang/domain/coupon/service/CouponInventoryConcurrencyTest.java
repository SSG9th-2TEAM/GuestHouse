...
class CouponInventoryConcurrencyTest {
    ...
    void setUp() {
        ...
        couponJpaRepository.save(testCoupon);

        // 재고 초기화
        CouponInventory inventory = couponInventoryRepository.findByCouponId(TEST_COUPON_ID)
                .orElse(CouponInventory.builder()
                        .couponId(TEST_COUPON_ID)
    ...
    void performanceComparison_RedisVsDb() throws InterruptedException {
        // Test 1: Redis + DB 하이브리드
        long hybridTime = measurePerformance(() -> 
            couponInventoryService.consumeSlotIfLimited(TEST_COUPON_ID), 100);
        ...
        long dbOnlyTime = measurePerformance(() -> 
            couponInventoryService.fallbackToDbLock(TEST_COUPON_ID), 100);
        ...
        // Redis + DB가 더 빨라야 함
        assertThat(hybridTime).isLessThan(dbOnlyTime);
    }
