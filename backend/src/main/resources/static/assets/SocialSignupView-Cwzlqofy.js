import{_ as se,r as t,d as oe,y as le,Y as te,m as ne,x as I,c as n,e,f,t as d,C as g,F as y,w as D,N as P,i as E,D as Y,q as S,Z as F,o as r}from"./index-ChMRK3m0.js";import{f as re,b as ae}from"./theme-B1xaNXbv.js";const ie={class:"register-page"},ce={class:"register-container"},de={class:"page-header"},ue={class:"progress-steps"},ve={class:"step-labels"},be={class:"terms-section"},me={class:"all-agree"},ge={class:"term-list"},he=["onUpdate:modelValue"],pe=["onClick"],ke=["disabled"],fe={class:"theme-section"},ye={key:0,class:"theme-loading"},_e={key:1,class:"theme-error"},we={key:2,class:"theme-categories"},Ce={class:"category-header"},Te={class:"category-emoji"},xe={class:"category-name"},Ae={class:"theme-chips"},Me=["onClick"],je=["src","alt"],Ie={class:"chip-label"},Ee={key:0,class:"chip-check"},Se={class:"final-actions"},Ne=["disabled"],Le=["disabled"],Ve={class:"modal-content"},qe={class:"modal-message"},Be={class:"modal-content large"},Ue={class:"terms-content-scroll"},Re=["innerHTML"],N=3,De={__name:"SocialSignupView",setup(Pe){const _=ne(),L=le(),c=t(1),v=t(!1);oe(async()=>{const o=L.query.accessToken,s=L.query.refreshToken;o&&s?(te(o,s),console.log("소셜 로그인 토큰 저장 완료"),await X()):(console.error("토큰이 없습니다. 로그인 페이지로 이동합니다."),_.push("/login"))});const w=t(!1),b=t([{id:1,label:"(필수) 서비스 이용약관",required:!0,checked:!1,content:`
    제1조 (목적)<br/>
    ...
    **동의하시는 경우, 아래와 같은 정보를 받아보실 수 있습니다.**<br/>
    ...
    `}]),H=()=>{b.value.forEach(o=>o.checked=w.value)},O=()=>{w.value=b.value.every(o=>o.checked)},V=I(()=>b.value.filter(o=>o.required).every(o=>o.checked)),x=t(!1),q=t(""),B=t(""),$=o=>{q.value=o.label,B.value=o.content,x.value=!0},A=()=>{x.value=!1},h=t([]),p=t(!1),k=t(""),C=t([]),z={NATURE:{emoji:"🌿",name:"자연"},CULTURE:{emoji:"🏛️",name:"문화"},ACTIVITY:{emoji:"🏄",name:"활동"},VIBE:{emoji:"✨",name:"분위기"},PARTY:{emoji:"🥳",name:"파티"},MEETING:{emoji:"💞",name:"만남"},PERSONA:{emoji:"👤",name:"특성/성향"},FACILITY:{emoji:"🏠",name:"시설"},FOOD:{emoji:"🍴",name:"음식"},PLAY:{emoji:"🎮",name:"놀이"}},U=I(()=>{if(C.value.length===0)return z;const o={};return C.value.forEach(s=>{o[s.categoryKey]={emoji:s.emoji,name:s.categoryName}}),o}),G=I(()=>{const o={};return h.value.forEach(s=>{o[s.category]||(o[s.category]=[]),o[s.category].push(s)}),o}),X=async()=>{p.value=!0,k.value="";try{const[o,s]=await Promise.all([re(),ae()]);s.ok&&Array.isArray(s.data)?(C.value=s.data,console.log("카테고리 로드 성공:",C.value)):console.warn("카테고리 목록 로드 실패, 기본값 사용:",s),o.ok&&Array.isArray(o.data)?(h.value=o.data.map(l=>({id:l.id,category:l.themeCategory,label:l.themeName,imageUrl:l.themeImageUrl,selected:!1})),console.log("테마 로드 성공:",h.value.length,"개")):(k.value="테마 목록을 불러오지 못했습니다.",console.error("테마 목록 로드 실패:",o))}catch(o){k.value="테마 목록을 불러오는 중 오류가 발생했습니다.",console.error("테마 목록 로드 에러:",o)}finally{p.value=!1}},Z=o=>{if(o.selected){o.selected=!1;return}if(h.value.filter(l=>l.selected).length>=N){u(`테마는 최대 ${N}개까지만 선택할 수 있습니다.`,"info");return}o.selected=!0},M=t(!1),R=t(""),J=t("info"),T=t(null),u=(o,s="info",l=null)=>{R.value=o,J.value=s,T.value=l,M.value=!0},j=()=>{M.value=!1,T.value&&(T.value(),T.value=null)},Q=()=>{c.value>1?c.value--:_.push("/login")},W=()=>{c.value===1&&(V.value?c.value=2:u("필수 약관에 동의해주세요.","error"))},K=async()=>{v.value=!0;try{const o=h.value.filter(m=>m.selected).map(m=>m.id),s=b.value.find(m=>m.id===3),l=s?s.checked:!1,a={termsAgreed:!0,themeIds:o.length>0?o:null,marketingAgreed:l};console.log("소셜 회원가입 데이터:",a);const i=await F(a);console.log("소셜 회원가입 응답:",i),i.ok&&i.data?(console.log("소셜 회원가입 완료!"),u("회원가입이 완료되었습니다!","success",()=>_.push("/"))):(console.error("소셜 회원가입 실패:",i),u(`회원가입에 실패했습니다.
